    ...
    for (var i in data) {
            ...
            indexRouter.push(
                {
                    ...
                    meta: {title: data[i].meta.title},
                    redirect: data[i].path,
                    children: [
                        ...generateChildRouters(data, false)
                    ]
                },
                {
                    path: '/outLink',
                    name: 'full',
                    //component: () => import('@/components/layouts/BasicLayout'),
                    component: resolve => require(['@/components/layouts/FullScreenLayout'], resolve),
                    meta: {title: '第三方'},
                    redirect: '/outLink'+data[i].path,
                    children: [
                        ...generateChildRouters(data, true)
                    ]
                },
                {
      ...
      /*  let indexRouter = [
            ...
            },
            {
                ...
                component: resolve => require(['@/components/layouts/FullScreenLayout'], resolve),
                meta: {title: '第三方'},
                redirect: '/outLink/dashboard/analysis',
                children: [
                    ...generateChildRouters(data, true)
                ]
            },
...
// 生成嵌套路由（子路由）
    ...
    function generateChildRouters(data, isFull) {
        const routers = []
        for (var item of data) {
            ...
            let fullPath = isFull ? '/out' : ''
            let fullName = isFull ? 'out_' : ''

            let menu = {
                path: fullPath + item.path,
            ...
            if (item.children && item.children.length > 0) {
                menu.children = [...generateChildRouters(item.children, isFull)]
            }
