...
export const HrefJump = {
  ...
  methods: {
    ...
    /** 处理列中的 href 跳转、dict 字典、长字符串缩略，使多种渲染可以兼容存在 */
    handleColumnHrefAndDictAndEllipsis(column = {}, fieldHrefSlotKeysMap = {}) {
      let { customRender, hrefSlotName } = column
      if (!hrefSlotName && (column.scopedSlots && column.scopedSlots.customRender)) {
        //hrefSlotName = column.scopedSlots.customRender
      }
        ...
        column.customRender = (text, record) => {
          ...
          if (hrefSlotName) {
            ...
            if (field) {
              // 此处为 JSX 语法
              // return (<a onClick={() => this.handleClickFieldHref(field, record)}>{value}</a>)
              return (<a onClick={() => this.handleClickFieldHref(field, record)}>
                <j-ellipsis length={column.fieldWidth == '' ? 25 : Number.parseInt(column.fieldWidth)} value={value}/>
              </a>)
            }
          }
          return (<j-ellipsis length={column.fieldWidth == '' ? 25 : Number.parseInt(column.fieldWidth)} value={value}/>)
        }
      }
    },
