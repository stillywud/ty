...
/** 忽略生成的字段 */
const IGNORES = ['id', 'create_by', 'create_time', 'update_by', 'update_time', 'sys_org_code', 'deleted']
...
function getBaseComponent(onlineField, params) {
  ...
  let objParams = {}
  if(['input','textarea','date','file-upload','imgupload','editor'].includes(params.type)){
    objParams = {
      cellLinkage:false,
      isCellLinkage:true
    }
  }else if (['select-user','select-depart','table-dict','table-dict_select'].includes(params.type)){
    objParams = {
      isCellLinkage:true
    }
  }else if(['radio','checkbox'].includes(params.type)){
    objParams = {
      cellLinkage:false,
      isCellLinkage:true,
      behaviorLinkage:[],
...
function packageSelect(onlineField) {
  ...
  if (dictField) {
    ...
    Object.assign(base.options, {
      ...
      props: {'value': 'value', 'label': 'label',primaryKey:''}, remote: 'dict',remoteFun:'',
      // -
      placeholder: '请选择' + params.name,
      dictCode: dictField,
    })
