<template>
  <a-card :bordered="false">
          ...
          <template slot="action" slot-scope="text, record">
            ...
            <template >
              <a-dropdown>
                ...
                <a-menu slot="overlay">
                  <!--表单设计器添加待办 by weina -->
                  <a-menu-item v-if="buttonsAuth.addtodo&&(record.todoStatus==1)">
    ...
    <j-import-modal ref="importModal" :url="url.importXls(desformCode)" :businessKeys="this.businessKeys" @ok="handleImportOk"/>

    <eoa-plan-modal ref="baseModal" :isurl="isurl" @ok="handleModalOk"/>
  </a-card>
</template>
...
<script>
  ...
  export default {
    ...
    components: { Ellipsis, AutoDesformDataFullScreen, AutoDesformDataModal, JSuperQuery, JImportModal,EoaPlanModal },
    data() {
      return {
        loading: false,
        isurl:true,
    ...
    watch: {
      $route: {
        ...
        handler(val) {
          ...
          if (val.path.indexOf('/online/desform/list/') === 0||val.path.indexOf('out/online/desform/list/') != -1) {
            this.reload()
          }
        },
      },
    ...
    methods: {
      ...
      loadData(desformCode) {
        ...
        return getAction(this.url.list, params).then((res) => {
            ...
            console.log('-------表单设计器 无权限跳转404 ----------',res)
            // 表单设计器 无权限跳转404 by weina
            if(res.code==500){
                this.$router.push({path:'@/views/exception/404'})
                return false
            }
