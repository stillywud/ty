...
<script>
  ...
  import { querySysCheckRule } from '@/api/api'
  ...
  export default {
    ...
    created() {
      ...
      querySysCheckRule(params).then((res) => {
        if(res.success){
          //找到验证规则下拉框数据源
          let options;
          ...
          //将系统编码校验规则添加到字段校验规则选项中，以’scr_‘开头标志
          res.result.records.forEach(item => {
            options.push({ title: item.ruleName, value: 'scr_'+item.ruleCode })
          })
        }
      })
    },
