<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="hdd" @click="recycleBinVisible=true" v-has="'user:edit'">回收站</a-button>
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              ...
              <template v-if="record.isDbSynch=='Y' && record.tableType !== 3">
                ...
                <a-menu-item>
                  <a @click="goAppPageOnline(record)">App功能测试</a>
                </a-menu-item>
                <a-menu-item>
    ...
    <a-modal
      ...
      <a-radio-group v-model="synMethod">
        ...
        <a-radio style="display: block;width: 30px;height: 30px" value="remarks">仅同步字段备注</a-radio>
      </a-radio-group>
    </a-modal>
    ...
    <a-modal
      ...
      <div style="display: flex; align-items: center;">
        <div style="flex: 1">Web地址：{{ onlineUrl }}</div>
        <a-button type="primary" class="copy-this-text" :data-clipboard-text="onlineUrl" @click="onCopyUrl">复制
        </a-button>
      </div>
      <div style="display: flex; align-items: center; margin-top: 10px;" v-if="onlineAppUrl!=''">
        <div style="flex: 1">App地址：{{ onlineAppUrl }}</div>
        <a-button type="primary" class="copy-this-text" :data-clipboard-text="onlineAppUrl" @click="onCopyUrl">复制
        </a-button>
      </div>
    </a-modal>
    ...
    <a-modal v-model="confirmVisible" :closable="false" :footer="null" :width="466">
      <div class="ant-modal-confirm-body">
        ...
        <span style="color: rgba(0, 0, 0, 0.85);font-weight: 500;font-size: 18px;">确认要移除吗？</span>
      </div>
    ...
    <!-- 回收站 -->
    <online-cgform-recycle-bin-modal :visible.sync="recycleBinVisible" @ok="modalFormOk"/>
  </a-card>
</template>
...
<script>
  ...
  import OnlineCgformRecycleBinModal from './modules/OnlineCgformRecycleBinModal'
    ...
    methods: {
      ...
      goAppPageOnline(rd) {
        ...
        let { href } = this.$router.resolve(
          {
            path: '/out/online/cgformAppList/' + rd.id + '?token=' + TOKEN
          }
        )
      ...
      handleOnlineUrlShow(record) {
        ...
        this.onlineAppUrl = `/out/online/cgformAppList/${record.id}`
