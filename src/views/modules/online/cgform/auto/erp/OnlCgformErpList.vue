  ...
  export default {
    ...
    methods:{
      ...
      onSelected(row){
        ...
        // 流程已提交，子表数据不能维护，隐藏编辑类按钮
        if (this.subList && this.subList.length>0 && row.bpm_status != '1' && row.bpm_status != '' && row.bpm_status != null) {
          let hideColumns = ['add','import','batch_delete','update','delete']
          this.subList.forEach(item => {
            this.$refs[item.currentTableName][0].subButtonSwitch({
              add:false,
              update:false,
              delete:false,
              batch_delete:false,
