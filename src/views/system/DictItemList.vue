          ...
          <a-form layout="inline" :form="form" @keyup.enter.native="searchQuery">
            ...
            <a-row>
              <a-col :md="10" :sm="24">
                ...
                <a-button type="primary" @click="setDefault" style="margin-left: 8px">设为默认</a-button>
              </a-col>
            </a-row>
          </a-form>
        </div>
    ...
    data() {
      return {
        ...
        url: {
          ...
          setDefault: "/sys/dict/setDefalut",
        },
      }
    },
    created() {
    ...
    methods: {
      ...
      setDefault() {
        if (this.selectedRowKeys.length <= 0) {
          ...
          params.defaultItem = this.selectedRowKeys[0]
          postAction(this.url.setDefault, params).then((res) => {
            if (res.success) {
              this.selectedRowKeys = [res.message]
              this.loadData()
            }
          })
        }
