    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="默认跳转地址">
          <a-input placeholder="请输入路由参数 redirect" v-decorator="[ 'redirect',validatorRules.redirect]" :readOnly="disableSubmit"/>
        </a-form-item>
...
<script>
  ...
  export default {
      ...
      validatorRules:function() {
        return {
          ...
          component:{rules: [{ required: this.show, message: '请输入前端组件!' }]},
            redirect:{rules: [{ required: false, message: '请输入路由参数 redirect!' }]},
    ...
    methods: {
      loadTree(){
        var that = this;
        queryTreeList({sysId:that.sysId}).then((res)=>{
          if(res.success){
      ...
      add (sysId) {
        this.sysId = sysId
        // 默认值
        this.edit({status:'1',permsType:'1',route:true,sysId:sysId});
      },
      edit (record) {
        this.sysId = record.sysId;
