...
<!--      </div>-->
    ...
    <a-modal
      ...
      <template slot="footer">
        <a-button type="primary" @click="groupOk">确认</a-button>
      </template>
      ...
      <a-form>
        <a-form-item
          ...
          :wrapperCol="{span:20}"
          style="margin-bottom:10px"
          :validate-status="group_validate_status">
          <a-tooltip placement="topLeft" >
            <template slot="title">
          ...
          <a-select @change="groupChange" :class="{'valid-error':group_validate_status=='error'}" placeholder="请选择登录租户" style="margin-left:10px;width: 80%">
            <a-icon slot="suffixIcon" type="gold" />
            <a-select-option
    ...
    methods: {
      ...
      handleSubmit () {
        ...
        if (that.customActiveKey === 'tab1') {
          that.form.validateFields([ 'username', 'password','inputCode','groupId', 'rememberMe' ], { force: true }, (err, values) => {
            if (!err) {
              loginParams.groupId = values.groupId
          ...
          let groupId = that.form.groupId
          that.form.validateFields([ 'mobile', 'captcha','rememberMe' ], { force: true }, (err, values) => {
            if (!err) {
              loginParams.groupId = groupId
      ...
      getCaptcha (e) {
        ...
        this.form.validateFields([ 'mobile'], { force: true },(err,values) => {
            if(!values.mobile){
              ...
              let smsParams = {};
              if(values.groupId){
                smsParams.groupId=values.groupId;
              }
      ...
      loginSuccess (msg) {
        ...
        var indexUrl =this.$store.state.user.indexUrl?this.$store.state.user.indexUrl:'/'; //如果state.user.indexUrl为空，默认指向'/'路径
        this.$router.push({ path:  indexUrl })
        this.$notification.success({
          message: '欢迎',
          description: `${timeFix()}，欢迎回来！` + msg,
        });
      },
      cmsFailed(err){
      ...
      groupOk(){
        if(!this.groupSelected){
          this.group_validate_status='error'
          return false
        }
        let obj = {
        ...
        let that = this;
        postAction("/sys/sms",obj).then(res=>{
          if(res.success){
            ...
            setTimeout(hide, 500);
            that.form.groupId = that.groupSelected
          }else{
            this.requestFailed(res)
          }
          this.groupClear()
      ...
      groupChange(value){
        this.group_validate_status='success'
