<template>
    <global-layout @dynamicRouterShow="dynamicRouterShow" :screenHeight="screenHeight" :navTopHeight="navTopHeight" :sideWidth="sideWidth" :asideHeight="asideHeight"
                   :navTophidden="navTophidden" :leftWidth="leftWidth" :collapsed="collapsed" >
        ...
        <!--layoutMode === 'sidemenu'-->
        <a-icon
                v-if="device!=='mobile'&& layoutMode === 'sidemenu'"
                class="trigger"
                :style="{left:this.leftWidth,top:menuIconTop} "
                style="position: absolute;color: #333;z-index: 999"
        ...
        <!--上下-->
        <a-icon
                v-if="device==='desktop'&& layoutMode === 'sidemenu'"
                class="trigger"
                :type="TopCollapsed? 'down-square' : 'up-square'"
                style="position: absolute;right:0px;color: #333;z-index: 999"
                :style="{top:posTop}"
                @click="Toptoggle"/>
        <a-tabs
        ...
        <!--<div :style="{'height': (screenHeight - 136) + 'px'}"-->
        <div :style="layoutStyle"
             style="margin: 12px 12px 0; overflow-y: auto; overflow-x: hidden">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            layoutStyle() {
                ...
                if(this.TopCollapsed){
                    style['height'] = (this.screenHeight-this.topHeight)+ 'px'
                }else{
                    style['height'] = (this.screenHeight -this.topHeight)+ 'px'
                }
                return style
        ...
        created() {
            // 首页配置
            this.indexKey = this.$store.state.user.indexUrl
            this.indexName = this.$store.state.user.indexName
        ...
        mounted() {
            ...
            this.asideHeight = window.innerHeight-70
            window.onresize = () => {
                return (() => {
                    this.screenHeight = document.body.clientHeight;
                    this.asideHeight = this.screenHeight-70
                })();
            };
        },
        watch: {
        ...
        methods: {
            ...
            toggle() {
                ...
                if(this.device==='desktop'){
                    if(this.layoutMode === 'sidemenu'){
                        this.sideWidth = this.collapsed ? '80px' : '200px';
                        if (this.collapsed) {
                            this.leftWidth = '70px';
                        } else {
                            this.leftWidth = '190px';
                        }
                    }
                }
            },
            Toptoggle() {
                ...
                if (this.TopCollapsed) {
                    this.navTopHeight = '0'
                    this.navTophidden = 'none'
                    this.posTop = '5px'
                    this.menuIconTop = '5px'
                    this.topHeight = 76
                    this.asideHeight=this.screenHeight-12
                } else {
                    this.navTopHeight = '59px'
                    this.navTophidden = 'block'
                    this.posTop = '63px'
                    this.menuIconTop = '63px'
                    this.topHeight = 136
                    this.asideHeight=this.screenHeight-70
                }
            ...
            changeTitle(title) {
                let projectTitle = Vue.ls.get("SYS_NAME") == null ? "天远开发平台" : Vue.ls.get("SYS_NAME");
                // 首页特殊处理
                if (this.$route.path === this.indexKey) {
