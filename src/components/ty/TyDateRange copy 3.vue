...
export default {
    ...
    data(){
        return {
            ...
            durationModel: this.models ? this.models[this.element.endModel] : '',
        }
    },
    ...
    //         },
    //         set(){
    //             this.startModel = this.models ? this.models[this.element.startModel] : ''
    //             this.endModel = this.models ? this.models[this.element.endModel] : ''
    //         }
    //     }
    // },
    methods:{
        ...
        inputC(e){
            console.log(e,3)
            let obj = {}
            obj[this.element.durationModel] = e
            this.$emit("update:dateRangeModel",obj)
        }
    },
    watch:{
            ...
            handler(val){
                ...
                if(this.rules){
                    ...
                    if(val){
                        ...
                        if(this.element.options.durationType){
                            copyRules[this.element.durationModel] = [{required:true,message:"请选择结束时间"}]  
                        }else{
                            copyRules[this.element.durationModel] = [{required:true,message:"请选择结束时间"}]
                        }
                    }else{
