  ...
  export default {
    ...
    methods:{
      ...
      handleImport() {
        ...
        if(this.businessKeys && this.businessKeys.length>0){
          formData.append('businessKeys',this.businessKeys);
        }
        if(this.isCover){
      ...
      handleChange(info) {
        if (this.fileNum != null){
          // // 文件数量限制
          this.fileList = [...this.fileList].slice(-(this.fileNum));
        }
      },
