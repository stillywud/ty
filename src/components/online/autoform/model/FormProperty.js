...
export default class FormProperty {
  ...
  get dictFilter(){
    return this.formSchema.dictFilter;
  }
  /*--------popup 三件套----------*/
  ...
  get rules(){
    ...
    if (this.formSchema.pattern) {
      ...
      }else if(this.formSchema.pattern.indexOf('scr_')==0){
        rules.push({
          validator: (r, v, c) => this.validateCheckRule(this.formSchema.pattern.slice(4), v, c)
        });
      }else{
  ...
  validate_check_rule=''
  get validateCheckRule(){
    return this.validate_check_rule ;
  }
  set validateCheckRule(value) {
    this.validate_check_rule = value;
  }
