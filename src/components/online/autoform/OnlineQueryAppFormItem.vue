<template>
  <a-form-item v-if="item.view=='date'" :label="item.label">
      <j-date :placeholder=" '请选择'+item.label " v-model="queryParam[item.field]" style="width: 100%"></j-date>
  </a-form-item>

  <a-form-item v-else-if="item.view=='datetime'" :label="item.label">
      <j-date :placeholder=" '请选择'+item.label " :show-time="true" style="width: 100%" date-format="YYYY-MM-DD HH:mm:ss" v-model="queryParam[item.field]"></j-date>
  </a-form-item>
  ...
  <a-form-item v-else :label="item.label">{{item.mode}}
