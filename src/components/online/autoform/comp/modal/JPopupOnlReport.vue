...
<script>
  ...
  export default {
    ...
    props: ['multi', 'code', 'groupId', 'dictFilter'],
    components:{
      OnlineQueryFormItem
    },
    data(){
    ...
    methods:{
      ...
      loadData(arg) {
        ...
        httpGroupRequest(() => getAction(url, params), groupIdKey).then(res => {
          ...
          if (data) {
            ...
            // 如果记录没有key字段，则以序号填充，避免数据重复时，复选框选择异常
            if (!data.records.key){
              data.records.map((item,index) => {item.key = index})
            }
      ...
      getQueryParams() {
        ...
        if (this.dictFilter){
          let paramPair = [];
          this.dictFilter.split(',').map(item => {
            paramPair = item.split('=');
            filterParams[paramPair[0]] = paramPair[1];
          })
        }
        let param = Object.assign({}, this.queryParam, this.sorter, filterParams);
