import Vue from 'vue'
import {login, logout, phoneLogin, thirdLogin, tokenRefresh, tokenLink} from "@/api/login"
import {
    ACCESS_TOKEN,
    REFRESH_TOKEN,
    USER_NAME,
...
const user = {
    ...
    actions: {
            ...
            return new Promise((resolve, reject) => {
                login(userInfo).then(response => {
                    if (response.code == '200') {
                        ...
                        const userInfo = result.userInfo
                        Vue.ls.set(ACCESS_TOKEN, result.token, 12 * 60 * 60 * 1000)
                        Vue.ls.set(REFRESH_TOKEN, result.refreshToken, 10 * 24 * 60 * 60 * 1000)
                        Vue.ls.set(USER_NAME, userInfo.username, 12 * 60 * 60 * 1000)
                        Vue.ls.set(USER_INFO, userInfo, 12 * 60 * 60 * 1000)
                        Vue.ls.set(UI_CACHE_DB_DICT_DATA, result.sysAllDictItems, 12 * 60 * 60 * 1000)
                        Vue.ls.set('DICT_SYS_NAME', result.dictSysName, 12 * 60 * 60 * 1000)
                        commit('SET_TOKEN', result.token)
        ...
        // 登录
        TokenLink({commit}, userInfo) {
            return new Promise((resolve, reject) => {
                tokenLink(userInfo).then(res => {
                   const resData=res.result
                    if (res.code == '200') {
                        Vue.ls.set(ACCESS_TOKEN, resData.token, 12 * 60 * 60 * 1000)
                        Vue.ls.set(USER_NAME, resData.user.username, 12 * 60 * 60 * 1000)
                        Vue.ls.set(USER_INFO, resData.user, 12 * 60 * 60 * 1000)
                        commit('SET_TOKEN', resData.token)
                        commit('SET_INFO', resData.user)
                        commit('SET_NAME', {
                            username: resData.user.username,
                            realname: resData.user.realname,
                            welcome: welcome()
                        })
                        commit('SET_AVATAR', resData.user.avatar)
                        resolve(res)
                    } else {
                        reject(res)
                    }
                    ...
                    if (menuData && menuData.length > 0) {
                        ...
                        menuData.forEach((item, index) => {
                            if(item.meta.title=='首页'){
                                commit('SET_INDEXURL', item.path)
                                commit('SET_INDEXNAME', item.name)
                            }
                            if (item["children"]) {
        ...
        // 刷新token
        TokenRefresh({commit}, param) {
            return new Promise((resolve, reject) => {
                tokenRefresh(param).then(response => {
                    if (response.code == '200') {
                        const result = response.result
