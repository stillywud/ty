    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="所属省" :labelCol="labelCol" :wrapperCol="wrapperCol">
		  <j-area-linkage type="cascader" v-decorator="['province']" placeholder="请输入省市区"/>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'addressTypeId','customerUid','contactsUid','addressInfo','addressDetailInfo','lat','lon','province','city','county','accessSysId','addressStatus'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'addressTypeId','customerUid','contactsUid','addressInfo','addressDetailInfo','lat','lon','province','city','county','accessSysId','addressStatus'))
      },
