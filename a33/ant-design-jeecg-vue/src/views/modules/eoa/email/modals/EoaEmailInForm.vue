      ...
      <div class="body">
        <div style="display: flex;">
          ...
          <div style="text-align: right;width: 50%">
            <a href="javascript:;" @click="toEmail()" >进入邮箱</a>
          </div>
        </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          ...
          editByEmailIdAndUserId:"/email/eoaMailboxInfo/editByEmailIdAndUserId",
    ...
    methods: {
      loadData(arg) {
        ...
        getAction(this.url.receiverPageList, {mailId:this.replyReocrd.content}).then((res) => {
          if (res.success) {
      ...
      view (record) {
        ...
        this.visible=true
        putAction(this.url.editByEmailIdAndUserId,{ id:record.id}).then((res) =>{})

        getAction(this.url.list,{id:record.id}).then((res) =>{
          if(res.success){
            this.replyReocrd = res.result.records[0];
