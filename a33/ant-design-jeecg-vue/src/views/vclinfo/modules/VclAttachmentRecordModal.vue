    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="属具设备机号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-dict-select-tag type="list" v-decorator="['attachedVclNo', validatorRules.attachedVclNo]" :trigger-change="true" dictCode="vcl_info,vcl_no,vcl_no" placeholder="请选择属具设备机号"/>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'vclNo','attachedVclNo','accessSysId','relRecordStatus'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'vclNo','attachedVclNo','accessSysId','relRecordStatus'))
      },
