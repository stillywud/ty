    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="上次保养日期" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-date placeholder="请选择上次保养日期" v-decorator="['vclLastservicedate']" :trigger-change="true" style="width: 100%"/>
        </a-form-item>
        <a-form-item label="是否已经保养" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-dict-select-tag type="list" v-decorator="['vclIsserviced', validatorRules.vclIsserviced]" :trigger-change="true" dictCode="dict_whether" placeholder="请选择是否已经保养"/>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'vclNo','vclNextservicehours','vclLastservicehours','vclLastservicedate','vclIsserviced','accessSysId','relRecordStatus'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'vclNo','vclNextservicehours','vclLastservicehours','vclLastservicedate','vclIsserviced','accessSysId','relRecordStatus'))
      },
