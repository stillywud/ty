<template>
  <a-card :bordered="false">
    <div style="padding-bottom:15px;display: inline-block;border-radius: 0 0 2px 2px;">
      <div style="display: inline-block">应用系统:</div>
      <j-search-select-tag v-model="sysId" @change="handleChange"
                           placeholder="请选择应用系统"
                           dict="sys_info,name,id,deleted=0 order by create_time" style="padding-left: 10px;width: 300px"/>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      loadData() {
        this.loading = true
        getSystemMenuList({"sysId":this.sysId}).then((res) => {
          if (res.success) {
      ...
      handleAddSub(record) {
        ...
        this.$refs.modalForm.edit({status:'1',permsType:'1',route:true,'parentId':record.id,sysId:this.sysId});
      },
      ...
      handleAdd: function () {
        this.$refs.modalForm.add(this.sysId);
        this.$refs.modalForm.title = "新增";
        this.$refs.modalForm.disableSubmit = false;
      },
      handleEdit: function (record) {
        record.sysId = this.sysId;
