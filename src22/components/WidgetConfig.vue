        ...
        <template v-if="data.options.remote === 'dict_obj'">
          ...
          <template v-if="!!data.dictObjOptions">
              <el-input size="mini" placeholder="请输入内容" class="el-mx-radg4-select">
                ...
                <el-select v-model="data.options.props.objValue" clearable slot="append" placeholder="请选择">
                  <el-option
              ...
              <el-input size="mini"  placeholder="请输入内容" class="el-mx-radg4-select">
                ...
                <el-select v-model="data.options.props.objLabel" clearable slot="append" placeholder="请选择">
                  <el-option
                        v-for="item in data.dictObjOptions"
                        :key="item.value"
                        :value="item.value"
              ...
              <el-input size="mini" v-model="data.options.props.primaryKey" placeholder="请输入外键" class="el-mx-radg4-input">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-select
                ...
                placeholder="请选择标签">
                <el-option
                  v-for="item in twOptions"
                  :key="item.fieldId"
                  :label="item.label"
                  :disabled="item.twdisabled"
                  :value="item.fieldId">
                </el-option>
              </el-select>
            </template>
  ...
  methods: {
    ...
    loadDictObjList() {
      ...
      _api.getDictObjList({
        ...
        this.dictObjOptions = packageOptions(res.result.records, 'dictName,dictCode', 'dictCode')
      })
    },
    loadCgformFieldList(headId) {
      _api.getOnlineFormFieldList({
    ...
    recurtwLinkage(op){
      ...
      function asd(op){
        that.list.forEach(it=>{
          let fils = it.type === 'select' && !it.options.multiple  && (it.options.remote === true || it.options.remote === 'dict_obj') // && !it.options.showLabel
          if(fils){
            // let valarr1 = []
    ...
    handleTwoLevelLinkage(){
      ...
      this.list.forEach(item => {
        if(
          item.type === "select" && 
          !item.multiple && 
          (item.options.remote === true || item.options.remote === 'dict_obj')
         ){
           let obj = {};
            obj.value = item.model
            obj.label = item.name
